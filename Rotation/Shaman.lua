...
function Shaman:DropTotems()
	...
	Shaman:CastTotem(ChooseAirTotem())

	if not MB_cooldowns["Tremor Totem"] then
		Shaman:CastTotem(ChooseEarthTotem())
	end

	if not MB_cooldowns["Poison Cleansing Totem"] then
		Shaman:CastTotem(ChooseWaterTotem())
	end

	if mb_tankTarget("Sapphiron") or mb_tankTarget("Azuregos") then
		Shaman:CastTotem(ChooseFireTotem())
	end
end

function Shaman:CastTotem(totem)
